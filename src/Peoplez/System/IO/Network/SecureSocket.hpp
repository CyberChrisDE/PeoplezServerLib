		...
		namespace IO
		{
			namespace Network
			{
				...
				public:
					 ...
					 */
					SecureSocket(int so, SSL* s) noexcept(std::is_nothrow_constructible<Socket>::value) : Socket(so), ssl(s) {}
					/**
					 ...
					 */
					virtual int Send(char const * buff, int len) noexcept;
					/**
					 ...
					 * Destructor
					 */
					virtual ~SecureSocket() noexcept {Close();}
				private:
					SecureSocket() = delete;
